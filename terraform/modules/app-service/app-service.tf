...
locals {
  ...
  appsp_plan_name     = "plan-${var.environment}-web"
  webapp_name         = "web-react-${var.environment}-${random_string.kv_suffix.result}"

  kv_name = "kv-${replace(var.environment, "_", "-")}-ascension-${random_string.kv_suffix.result}"
